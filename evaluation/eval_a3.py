...
class A3AblationEvaluator:
    ...
    def parse_test_sample_a3(self, test_sample):
        if isinstance(test_sample, dict):
            multimodal_input = test_sample.get('multimodal_input_asr', '')
            expected_output = test_sample.get('training_target', '')
            return multimodal_input, expected_output
        return "", ""
    ...
    def generate_response_a3(self, user_input: str) -> str:
        prompt = (
    ...
    def run_evaluation(self) -> Dict[str, Any]:
        ...
        for test_sample in test_data:
            user_input, expected_output = self.parse_test_sample_a3(test_sample)
            expected_orientation = self.extract_expected_orientation(test_sample)

            try:
                predicted_output = self.generate_response_a3(user_input)
        ...
        evaluation_results = {
            'method': 'A3 - ASR + Coordinates + Direct Prediction',
        ...
        with open('a3_evaluation_results.json', 'w', encoding='utf-8') as f:
