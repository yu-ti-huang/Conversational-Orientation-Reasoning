...
class A2AblationEvaluator:
    """A2 Ablation Study: ASR Text Only (No Coordinates)"""
    ...
    def parse_test_sample_a2(self, test_sample):
        if isinstance(test_sample, dict):
            asr_text = test_sample.get('asr_text', '')
            expected_output = test_sample.get('training_target', '')
            return asr_text, expected_output
        return "", ""

    def extract_expected_orientation(self, test_sample):
        if isinstance(test_sample, dict) and 'target_direction' in test_sample:
    ...
    def generate_response_a2(self, user_input: str) -> str:
        prompt = (
            "<|begin_of_text|><|start_header_id|>user<|end_header_id|>\n\n"
            f"{user_input}\n\n"
            "請只回答使用者面朝的方向，只需要一個字：北、南、東、西。"
    ...
    def run_evaluation(self) -> Dict[str, Any]:
        ...
        for test_sample in test_data:
            user_input, expected_output = self.parse_test_sample_a2(test_sample)
            expected_orientation = self.extract_expected_orientation(test_sample)

            try:
                predicted_output = self.generate_response_a2(user_input)
        ...
        orientation_accuracy = correct_orientations / total_samples if total_samples else 0.0
        format_error_rate = format_errors / total_samples if total_samples else 0.0

        evaluation_results = {
            'method': 'A2 - ASR Text Only + Direct Prediction',
        ...
        with open('a2_evaluation_results.json', 'w', encoding='utf-8') as f:
...
def main():
    evaluator = A2AblationEvaluator(csv_path="./spatial_test.csv")
    results = evaluator.run_evaluation()
    print("\n" + "="*60)
    print("FINAL A2 RESULTS FOR PAPER:")
    print(f"A2 Orientation Accuracy: {results['metrics']['orientation_accuracy']:.3f}")
    print(f"A2 Format Error Rate: {results['metrics']['format_error_rate']:.3f}")
    print("="*60)
